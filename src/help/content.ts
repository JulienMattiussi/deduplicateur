...
**Miniatures dans le comparateur** : pour les entrées image, une **miniature** remplace l'icône de type de fichier. Le chargement est **paresseux** : la miniature est générée à la volée quand la ligne entre dans la zone visible (extraction de l'entrée en mémoire + redimensionnement en JPEG base64). **Cliquer sur une miniature** ouvre l'image dans le viewer par défaut du système : l'entrée est extraite vers un fichier temporaire (sous-dossier \`archive_preview/\` du dossier de données de l'app), un spinner overlay s'affiche pendant l'opération, puis l'image s'ouvre. Les fichiers temporaires sont **purgés automatiquement** au prochain démarrage de l'app.
...
**Thumbnails in the comparator**: for image entries, a **thumbnail** replaces the file-type icon. Loading is **lazy**: the thumbnail is generated on-the-fly when the row enters the visible area (in-memory entry extraction + JPEG base64 resize). **Clicking on a thumbnail** opens the image in the system's default viewer: the entry is extracted into a temporary file (in the \`archive_preview/\` subfolder of the app data directory), a spinner overlay shows during the operation, then the image opens. Temporary files are **automatically purged** at the next app launch.
