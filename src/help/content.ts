...
**Archive comparator**: click **Compare** to open the full-screen comparator. Entries are displayed **face to face** in a 3-column layout: archive A on the left (size on the outer edge, thumbnail on the inner edge), the **match score** in the center (\`100%\` for an exact duplicate, or a similarity percentage for close images), archive B on the right (thumbnail on the inner edge, size on the outer edge). For image entries, a **thumbnail** is shown instead of the file-type icon. A **"Duplicates only"** checkbox hides non-shared entries. At the bottom of the screen, a meta block per archive shows name, folder, size, date and duplicated/total entry ratio.
