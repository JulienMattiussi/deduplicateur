...
**Zoom et déplacement** : le **curseur loupe** affiché au survol d'une image indique que vous pouvez zoomer. Utilisez la **molette de la souris** pour zoomer (jusqu'à 10x). Le zoom est centré sur la position du curseur et **synchronisé entre les deux images** : faites pivoter la molette sur l'une, les deux zooment ensemble. Une fois zoomé, le curseur devient une main : cliquez-glissez pour **déplacer la vue** - les deux images suivent. Le zoom est réinitialisé en fermant et rouvrant le comparateur ; il est conservé en changeant d'onglet ou en naviguant entre groupes.
...
**Zoom and pan**: the **magnifier cursor** shown when hovering an image indicates zoom is available. Use the **mouse wheel** to zoom (up to 10x). Zoom is centered on the cursor position and **synchronised between both images**: spin the wheel on one, both zoom together. Once zoomed, the cursor becomes a hand: click-and-drag to **pan the view** - both images follow. Zoom is reset when closing and reopening the comparator; it is preserved when switching tabs or navigating between groups.
