...
pub async fn get_archive_comparison(
    ...
    // Lecture cache : si le scan a deja calcule les hashes (xxh3 + pHash) des entrees,
    // on construit la comparaison sans rouvrir / redecoder les archives.
    let cached = {
    ...
    if let Some((entries_a, entries_b)) = cached {
        return Ok(build_comparison_from_cache(
            &path_a, &path_b, &entries_a, &entries_b, find_similar, sim_threshold,
        ));
    }

    // Fallback : pas de cache (ex. session pre-cache loadee depuis une vieille version).
    // On re-calcule via le chemin lent.
    tauri::async_runtime::spawn_blocking(move || {
